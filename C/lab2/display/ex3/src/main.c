...
// Define os enables dos displays
...
#define DEC_BUTTON PORTBbits.RB1
#define COUNT_BUTTON PORTBbits.RB2
...
void main(void) {
    ...
    while (1) {
        ...
        // === Botao de contagem ===
        if (!COUNT_BUTTON) {
            contando = !contando; // Para poder parar e inciar quando quiser
        ...
        if (contando && valor > 0) {
            valor--;
            // delay_1s();
            // Garante atualização dos displays por ser polling
            for (int i = 0; i < 100; i++) {
                mostra_valor(valor);
                __delay_ms(10);
            }
            if (valor == 0) contando = 0;
        }
    }
}
...
void mostra_valor(int valor) {
    ...
    // Quebra o valor em unidades, dezenas, centenas e milhar
    milhar  = valor / 1000;
    centena = (valor / 100) % 10;
    dezena  = (valor / 10) % 10;
    unidade = valor % 10;

    // Multiplexação simples (4 displays)
    DISPLAY1_ENABLE = 1; DISPLAY2_ENABLE = 0; 
    DISPLAY3_ENABLE = 0; DISPLAY4_ENABLE = 0;
    DISPLAY_VALUE = display_values[milhar];
    __delay_ms(2);

    DISPLAY1_ENABLE = 0; DISPLAY2_ENABLE = 1;
    DISPLAY3_ENABLE = 0; DISPLAY4_ENABLE = 0;
    DISPLAY_VALUE = display_values[centena];
    __delay_ms(2);

    DISPLAY1_ENABLE = 0; DISPLAY2_ENABLE = 0;
    DISPLAY3_ENABLE = 1; DISPLAY4_ENABLE = 0;
    DISPLAY_VALUE = display_values[dezena];
    __delay_ms(2);

    DISPLAY1_ENABLE = 0; DISPLAY2_ENABLE = 0;
    DISPLAY3_ENABLE = 0; DISPLAY4_ENABLE = 1;
