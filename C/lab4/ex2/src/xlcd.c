...
Alterada por: Marcelo Martins Maia do Couto 
			  21/03/2012 (Adaptação da rotina para o LCD FDCC1602E-NSWBBW-51SE)
			  marcelo@exsto.com.br 
...
// Setup interface to LCD - FIRST BYTE
#ifdef BIT8                             // 8-bit mode interface
    TRIS_DATA_PORT = 0;             // Data port output
    DATA_PORT = 0b00111000;         // Function set cmd(8-bit interface)
...
// Setup interface to LCD - SECOND BYTE
#ifdef BIT8                             // 8-bit interface
    DATA_PORT = 0b00111000;         	// Function set cmd (8-bit interface, 2 lines, 5x8 dots)
...
#endif
        E_PIN = 1;                      // Clock the cmd in
        DelayFor18TCY();				// Delay for Initialization...
        E_PIN = 0;

        // Delay for at least 100us
        DelayXLCD();

// Setup interface to LCD - THIRD BYTE
#ifdef BIT8                             // 8-bit interface
        DATA_PORT = 0b00111000;         // Function set cmd (8-bit interface, 2 lines, 5x8 dots)
#else                                   // 4-bit interface
	#ifdef UPPER                        // Upper nibble interface
...
// ATENÇÃO: Este bloco abaixo está comentado porque o LCD FDCC1602E-NSWBBW-51SE não responde ao bit de busy
...
//        }
        ...
        // ? CORREÇÃO: Adicionar return quando BIT8 está definido
        // Como o LCD não responde ao busy bit, sempre retorna 0 (não ocupado)
        E_PIN = 0;                      // Reset clock line
        RW_PIN = 0;                     // Reset control line
        return 0;                       // Return FALSE (LCD sempre pronto)
